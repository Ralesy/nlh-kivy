#!/usr/bin/env python3
"""Test for RoamingEntityManager (chase cooldown, zone-bound, respawn, 120px cone)."""
import sys, math
sys.path.insert(0, ".")
from systems.roaming_entity_manager import RoamingEntityManager
from core.models.roaming_token import TokenState, CONTACT_RADIUS, CONE_LENGTH, HEARING_RADIUS

MAP_W, MAP_H = 1280, 720
rm = RoamingEntityManager(map_world_width=MAP_W, map_world_height=MAP_H)
rm.set_map_size(MAP_W, MAP_H)
print(f"Zones: {len(rm.zones)}, Tokens: {len(rm.tokens)}")
...
tk3._direction_x = 0.0
tk3._direction_y = -1.0
spx = tk3.x
spy = tk3.y + 30.0
for step in range(5):
